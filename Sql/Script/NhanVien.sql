...
--------------------------------------------------------
ALTER TABLE NHANVIEN ADD CONSTRAINT NV_TK_FK FOREIGN KEY (TenDangNhap)
...
AS
BEGIN
    Insert into TAIKHOAN (TenDangNhap,MatKhau,Role,TinhTrang)
        values (TDN, MK, 1, 'Hoạt động');
        
    Insert into NHANVIEN (ID_NhanVien,ID_NQL, TenDangNhap, TenNV, GioiTinh, NgaySinh, NgayVaoLam, SDT, Email,TinhTrang)  
        values (NHANVIEN_ID.NEXTVAL, MANQL, TDN, TNV, GT, NGSINH, NGVL, SODIENTHOAI, MAIL, 'Hoạt động');
...
AS
    TDN NHANVIEN.TenDangNhap%TYPE;
BEGIN
        UPDATE NHANVIEN SET TinhTrang='Đã hủy'
        WHERE ID_NhanVien=MANV;
        
        SELECT TenDangNhap INTO TDN
        FROM NHANVIEN
        WHERE ID_NhanVien=MANV;
